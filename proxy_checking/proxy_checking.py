...
class ProxyChecker:
    ...
    async def get_info_async(self, url=None, proxy=None):
        info = {}
        proxy_type = []
        judges = [
            "http://proxyjudge.us/azenv.php",
    ...
    async def check_proxy_async(self, proxy):
        ip = proxy.split(":")
        resp = self.get_info(proxy=proxy)
        ...
        if resp["status"] == True:
            ...
            result["time_response"] = resp["time_response"]
            result["anonymity"] = resp["anonymity"]
            result["country"] = geo_info["country"] if "country" in geo_info else ""
            result["city"] = geo_info["city"] if "city" in geo_info else ""
            try:
                result["country_code"] = geo_info["country_code"]
            except:
                result["country_code"] = (
                    geo_info["countryCode"] if "countryCode" in geo_info else ""
                )
